...
func (uc *UserCreate) AddNotificationChangeIDs(ids ...int) *UserCreate {
	uc.mutation.AddNotificationChangeIDs(ids...)
	return uc
}

// AddNotificationChanges adds the "notificationChanges" edges to the NotificationChange entity.
func (uc *UserCreate) AddNotificationChanges(n ...*NotificationChange) *UserCreate {
	ids := make([]int, len(n))
	for i := range n {
		ids[i] = n[i].ID
	}
	return uc.AddNotificationChangeIDs(ids...)
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.NotificationChangesIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
