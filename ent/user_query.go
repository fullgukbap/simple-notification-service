...
func (uq *UserQuery) QueryNotificationChanges() *NotificationChangeQuery {
	query := (&NotificationChangeClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) WithNotificationChanges(opts ...func(*NotificationChangeQuery)) *UserQuery {
	query := (&NotificationChangeClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withNotificationChanges = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withNotificationChanges; query != nil {
		if err := uq.loadNotificationChanges(ctx, query, nodes,
			func(n *User) { n.Edges.NotificationChanges = []*NotificationChange{} },
			func(n *User, e *NotificationChange) {
...
func (uq *UserQuery) loadNotificationChanges(ctx context.Context, query *NotificationChangeQuery, nodes []*User, init func(*User), assign func(*User, *NotificationChange)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.NotificationChange(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.NotificationChangesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_notification_changes
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_notification_changes" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_notification_changes" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
