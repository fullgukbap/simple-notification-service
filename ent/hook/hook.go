...
// The NotificationChangeFunc type is an adapter to allow the use of ordinary
// function as NotificationChange mutator.
type NotificationChangeFunc func(context.Context, *ent.NotificationChangeMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f NotificationChangeFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.NotificationChangeMutation); ok {
		return f(ctx, mv)
	}
