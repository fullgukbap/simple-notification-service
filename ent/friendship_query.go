...
func (fq *FriendshipQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Friendship, error) {
	...
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, friendship.ForeignKeys...)
	}
...
func (fq *FriendshipQuery) loadSenderID(ctx context.Context, query *UserQuery, nodes []*Friendship, init func(*Friendship), assign func(*Friendship, *User)) error {
	...
	for i := range nodes {
		if nodes[i].user_friendships_receiver == nil {
			continue
		}
		fk := *nodes[i].user_friendships_receiver
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_friendships_receiver" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (fq *FriendshipQuery) loadReceiverID(ctx context.Context, query *UserQuery, nodes []*Friendship, init func(*Friendship), assign func(*Friendship, *User)) error {
	...
	nodeids := make(map[int][]*Friendship)
	for i := range nodes {
		if nodes[i].user_friendships_sender == nil {
			continue
		}
		fk := *nodes[i].user_friendships_sender
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_friendships_sender" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (fq *FriendshipQuery) loadFriendshipStatusID(ctx context.Context, query *FriendshipStatusQuery, nodes []*Friendship, init func(*Friendship), assign func(*Friendship, *FriendshipStatus)) error {
	...
	nodeids := make(map[int][]*Friendship)
	for i := range nodes {
		if nodes[i].friendship_status_friendships == nil {
			continue
		}
		fk := *nodes[i].friendship_status_friendships
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	query.Where(friendshipstatus.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "friendship_status_friendships" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
